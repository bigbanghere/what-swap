 ...
 * Creates all tables and initial data via Supabase API
 */
...
async function insertDefaultTemplates() {
  ...
  for (let i = 0; i < templates.length; i++) {
    ...
    console.log(`Inserting template ${i + 1}/${templates.length}: ${template.name}`);
...
async function main() {
  ...
  const proceed = await askQuestion('This will create tables in your Supabase database. Continue? (y/N): ');
